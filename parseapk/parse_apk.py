...
def parse_apk(data):
    ...
    '''
    for eocd in re.finditer(b"PK\5\6", data):
        parse_eocd(data, eocd.start())

    for cd in re.finditer(b"PK\1\2", data):
        parse_cd(data, cd.start())

    for lfh in re.finditer(b"PK\3\4", data):
        parse_file(data, lfh.start())

    for sb in re.finditer(b"APK Sig Block 42", data):
...
def str_algo_id(algo_id):
    ...
    if algo_id == 0x00000421:
        return "VERITY RSA PKCS1-v1_5 with SHA2-256"
    if algo_id == 0x00000423:
        return "VERITY ECDSA with SHA2-256"
    if algo_id == 0x00000425:
        return "VERITY DSA with SHA2-256"
...
def parse_signatures(data, offset):
    ...
    while i < (total_length + 4):
        ...
        print("\t\t\t\tSignature: ", binascii.hexlify(sig)[:20].decode('utf-8')+'...')
...
def parse_digest(data, offset):
    ...
    print("\t\t\t\t\tDigest: ", binascii.hexlify(digest)[:20].decode('utf-8')+'...')
...
def parse_certificates(data, offset, length):
    ...
    while i < length:
        ...
        print("\t\t\tASN.1 DER certificate: ", binascii.hexlify(certificate[:10]).decode('utf-8')+'...')
        nb = nb + 1
